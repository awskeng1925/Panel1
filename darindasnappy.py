...
# Track message count per node for group name rotation
message_counter = {}
group_name_rotator = {}
group_name_cycle = {}
        ...
        try:
            ...
            while lock_name_threads.get(module_key, False):
                try:
                    ...
                    log_event(f"[{uname}] NC Lock error: {str(inner_ex)[:40]}", "warning")
                    time.sleep(8)
        except Exception as e:
...
def run_spam_worker(session_id, initial_target, custom_texts_list, template_list, target_scope, target_gc_input, custom_delay, module_key, uname):
    ...
    if uname not in message_counter:
    ...
    while active_spam_threads.get(module_key, False):
        ...
        try:
            ...
            try:
                ...
                log_event(f"[{uname}] Session expired, attempting refresh...", "warning")
                cl, acc_info = refresh_session(session_id)
                if not cl or not acc_info:
                    log_event(f"[{uname}] Session refresh failed, sleeping...", "error")
                    time.sleep(30)
                    continue
            
            log_event(f"[{uname}] Authenticated with Jitter Pacing & Load-Balancing.", "success")
                    ...
                    for thread_id in target_threads:
                        ...
                        try:
                            ...
                            if "block" in err_msg.lower() or "limit" in err_msg.lower() or "item_ack" in err_msg.lower():
                                log_event(f"[{uname}] SPAM DETECTED! Auto-Healer shifting node to 60s quarantine rest...", "error")
                                
                                node_quarantine_status[uname] = True
                                def release_quarantine(u):
                                    time.sleep(60)
                                    node_quarantine_status[u] = False
                                    log_event(f"[{u}] Quarantine finished. Node back in rotation!", "success")
                                
                                q_thread = threading.Thread(target=release_quarantine, args=(uname,))
                                q_thread.daemon = True
                                q_thread.start()
                                break
                            elif "login" in err_msg.lower() or "session" in err_msg.lower():
                                log_event(f"[{uname}] Session error, refreshing...", "warning")
                                cl, acc_info = refresh_session(session_id)
                                if not cl:
                                    time.sleep(15)
                            else:
            ...
            log_event(f"[{uname}] Session Sync Error. Re-connecting in 10s...", "error")
            time.sleep(10)
...
<div class="container">
    <div class="header">
        <h1>SIREN CLAN &mdash; Operator: {{ session.get('operator_name', 'ELITE') }}</h1>
...
<div class="grid-2">
    <div>
        ...
        <div class="card">
            <div class="card-title">Upload Automa JSON Script</div>
            <form method="POST" enctype="multipart/form-data">
                <input type="hidden" name="action_type" value="upload_automa_json">
                <div class="form-group">
                    <label>Select Automa Workflow JSON File</label>
                    <input type="file" name="automa_file" accept=".json" required style="padding: 6px;">
                </div>
                <button type="submit" class="btn btn-warning">Upload & Parse JSON</button>
            </form>
        </div>
